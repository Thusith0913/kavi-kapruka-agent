...
const CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .cart-item-img{width:48px;height:48px;object-fit:cover;border-radius:6px;flex-shrink:0;}
  ...
  /* Product carousel */
  .products-carousel{display:flex;gap:10px;overflow-x:auto;padding:10px 0 6px;scroll-snap-type:x mandatory;max-width:520px;}
  ...
  .form-submit{flex:1;padding:11px;background:linear-gradient(135deg,var(--saffron),var(--coral));color:#0f0c0a;border:none;border-radius:50px;font-weight:700;font-size:14px;cursor:pointer;transition:opacity 0.2s,transform 0.2s;font-family:'DM Sans',sans-serif;}
  .form-submit:hover{opacity:0.9;transform:scale(1.01);}
  .form-submit:disabled{opacity:0.4;cursor:default;}
  .form-skip{padding:11px 18px;background:transparent;border:1px solid var(--border);border-radius:50px;color:var(--muted);font-size:13px;cursor:pointer;transition:all 0.2s;font-family:'DM Sans',sans-serif;}
  .form-skip:hover{border-color:var(--muted);color:var(--text);}
  ...
  .order-summary-row value{color:var(--text);font-weight:500;text-align:right;max-width:200px;}
...
function useCart() {
  ...
  const changeQty = (id, d) => setItems(prev =>
    prev.map(i => i.id === id ? { ...i, qty: Math.max(1, i.qty + d) } : i).filter(i => i.qty > 0)
  );
...
function DeliveryForm({ onSubmit, onSkip }) {
  ...
  const filled = Object.entries(form).filter(([k]) => k !== "note" && k !== "date").every(([, v]) => v.trim());
      ...
      <div className="delivery-form">
        <div className="delivery-form-header">
          ...
          <div>
            ...
            <div className="delivery-form-sub">Fill in to complete your order — takes 30 seconds!</div>
          </div>
        </div>
        ...
        <div className="delivery-form-body">
          <div className="form-row">
            <div className="form-field">
              <div className="form-label"><span>👤</span> Recipient Name *</div>
              <input className="form-input" placeholder="e.g. Thusitha" value={form.name} onChange={set("name")} />
            </div>
            <div className="form-field">
          ...
          <div className="form-row">
            ...
            <div className="form-field">
              <div className="form-label"><span>📅</span> Delivery Date</div>
          ...
          <div className="form-field">
            ...
            <textarea className="form-input form-textarea" placeholder="Add a personal note to make it extra special..." value={form.note} onChange={set("note")} />
          </div>
        </div>
...
// ─── System prompt ───────────────────────────────────────────────────────────
const buildSystem = (lang) => `You are Kavi — Sri Lanka's most beloved AI shopping companion for Kapruka.com. Warm, witty, emotionally intelligent, culturally aware.

${lang === "si"
  ? `LANGUAGE: Respond in Sinhala mixed with English naturally. Use Sinhala script. Example: "ආයුබෝවන්! 🙏 Machan, mokakda gift eka?"`
  : `LANGUAGE: Respond in friendly Singlish — mix English with Sinhala naturally. Use "Aiyo!", "Machan", "Pako ne?", "Aney", "Chee!" where it fits.`}

PERSONALITY:
- Read the emotional situation first. Breakup? Sympathy then flowers. Birthday panic? "Kavi's got this!"
- Have opinions. Recommend confidently. "If I were you, I'd go with this one."
- Mention Avurudu, Vesak, Poya days, local customs when relevant.
- MOST users shop for THEMSELVES — groceries, electronics, fashion, daily needs. Serve both gifting and self-shopping equally.

IMPORTANT — DELIVERY FORM:
When a user is ready to checkout or asks for delivery/order, tell them you'll collect their details, then output this EXACT trigger (nothing else on that line):
[SHOW_DELIVERY_FORM]

The UI will show an interactive form asking for: recipient name, phone, address, city, delivery date, gift message. After they fill it, you'll get the details and can create the order.

FORMATTING — embed structured data as JSON blocks:
...
For delivery info:
\`\`\`json:delivery
{"City":"Colombo","Date":"2026-06-10","Fee":"LKR 350","Available":"Yes ✅"}
\`\`\`
...
Keep prose to 2-3 sentences. Never make up products or prices.`;
...
// ─── Constants ───────────────────────────────────────────────────────────────
const WELCOME_CHIPS = [
  "💔 I just broke up — send flowers",
  "🎂 Birthday gift under LKR 3,000",
...
export default function KaprukaAgent() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      // Check if agent wants to show delivery form
      if (reply.includes("[SHOW_DELIVERY_FORM]")) {
        const prose = reply.replace("[SHOW_DELIVERY_FORM]", "").trim();
        const agentMsg = { role: "assistant", content: prose || "Great! Let me collect your delivery details." };
        setMessages([...newHistory, agentMsg, { role: "assistant", content: "__DELIVERY_FORM__" }]);
      } else {
  ...
  const handleFormSubmit = useCallback(async (formData) => {
    // Replace form message with order summary
    setMessages(prev => prev.map(m =>
      m.content === "__DELIVERY_FORM__"
        ? { role: "assistant", content: `__ORDER_SUMMARY__${JSON.stringify(formData)}` }
        : m
    ));
    // Send to Kavi for checkout
    const cartSummary = cart.items.length
      ? `Cart: ${cart.items.map(i => `${i.qty}x ${i.name} (${i.price})`).join(", ")}. `
      : "";
    const msg = `${cartSummary}Delivery to: ${formData.name}, ${formData.phone}, ${formData.address}, ${formData.city}.${formData.date ? ` Date: ${formData.date}.` : ""}${formData.note ? ` Gift message: "${formData.note}".` : ""} Please confirm and give me the checkout/pay link.`;
  ...
  const handleCartCheckout = useCallback(() => {
    setCartOpen(false);
    setMessages(prev => [...prev, { role: "assistant", content: "__DELIVERY_FORM__" }]);
  }, []);
  ...
  return (
    <div className="app">
      ...
      <div className="main">
        ...
        <CartPanel cart={cart} open={cartOpen} onClose={() => setCartOpen(false)} onCheckout={handleCartCheckout} />
      </div>
    </div>
  );
}
